  ...
  return (
    <header className="fixed left-0 top-0 md:ml-64 w-full md:w-[calc(100%-256px)] bg-[#0A0A0A]/90 flex items-center justify-between p-4 z-40">
      ...
      <div className="flex items-center gap-6">
        ...
        {localStorage.getItem("userData") ? "" : "Sign Up" }
        </Link>
        <Link
          to="/signin"
        ...
        >
          {localStorage.getItem("userData") ? JSON.parse(localStorage.getItem("userData")).user.email : "Sign In" }
        </Link>
        <button className="py-2 md:py-3 px-4 rounded-full text-side-bub bg-white font-medium hover:scale-105 transition-transform text-black" onClick={handleLogout}>
          Logout
        </button>
      </div>
    </header>
