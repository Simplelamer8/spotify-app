...
class AuthService {
  ...
  async updateUser(email, updateProperty, updateValue){
    const user = await UserModel.findOne({email});
    if (!user) return null;
    user[updateProperty] = updateValue;
  ...
  async getGroupmates(userData){
    const {groupNumber} = userData.user;
    const users = await UserModel.find({groupNumber});
    return users;
  }
